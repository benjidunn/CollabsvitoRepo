...
# Delete old-LOGfile if it is in current Directory
if os.path.isfile("Z_logging(DELETE).txt"):
    os.remove("Z_logging(DELETE).txt")
# create new-LOGfile in current-Directory
sys.stdout=open("Z_logging(DELETE).txt","w")
print (datetime.datetime.now().strftime("ERSTELLT <> Datum: %d-%m-%Y <> Uhrzeit: %H.%M.%S <>"))
print ('>>>')
...
if (__name__ == "__main__"):
    ...
    for x in filesinrootdir:
        if re.search("plugin|repository|script", x) and not x.endswith('.zip'):
...
#time.sleep(60)
sys.stdout.close()
os.system('attrib +h Z_logging(DELETE).txt')